...
- [ ] `#34`: LightGCN bipartite(#35) vs tri 비교로 페르소나 순수 효과 분리 측정 — **최종 확정 조합**(구매만, emb_dim=32, lr=0.002, lamda=0.02, layer=2, Adam, batch=10000, uniform, neg_samples=10, HR@20≈0.0453)을 bipartite에도 동일 적용해야 공정한 비교, 대시보드 Tab2(페르소나 기여도) 요구사항과 직결 — `#34` 코멘트 갱신 예정
