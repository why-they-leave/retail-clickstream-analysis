...
>
> **중요 업데이트 (같은 날, #37 — CodeRabbit 실버그 발견)**: `#30` PR 리뷰에서 CodeRabbit이 `model_LightGCN_tri.py`의 레이어 결합 가중치가 "표준 LightGCN"이라는 주석과 달리 실제로는 레이어0(원본 임베딩)에 편중돼 있었다는 걸 지적했다. **1~4차의 모든 결과는 이 버그 위에서 나온 것이었다.** 표준 방식(균등 평균)으로 고치고 재실행하니 HR@20이 0.0553 → 0.0444로 오히려 낮아졌다 — 버그가 우연히 이 데이터에 더 잘 맞았던 것. 코드는 "표준 LightGCN" claim에 맞게 고치되, 편중 방식은 `--layer-weight-scheme decay` 정식 옵션으로 남겨 계속 비교 대상으로 뒀다. 상세는 [5차 실행](#분석-결과--5차-실행-버그-수정-후-재검증-37) 참고.
>
> **재현성 정정 (같은 날, 5-4)**: 5차에서 `neg_samples=10, lr=0.002`가 HR@20=0.0505로 최고치였다고 발표했는데, negative sampling에 seed를 안 걸어놔서 **재현이 안 됐다**. 3회 반복 실행으로 검증하니 평균 0.0453(±0.0022) — 이상치였던 0.0505를 정정한다. **최종 확정 수치는 HR@20≈0.0453, ALS 대비 격차 약 1.34배**다. 상세는 [5-4 재현성 검증](#5-4-재현성-검증--neg10lr0002의-00505는-이상치였다) 참고.
...
**방법론적 주의(#37 리뷰 지적)**: uniform은 레이어 가중치 합이 항상 1이지만(layer=2 기준 0.333×3=1.0), decay는 합이 1이 아니다(1+0.5+0.333≈1.83) — 예전 버그 동작을 그대로 재현하려고 정규화하지 않았기 때문이다. 그래서 이 uniform vs decay 비교는 "레이어별 가중치 분포 형태" 차이만 보는 게 아니라 "최종 임베딩의 전체 스케일" 차이도 함께 섞여 있는 결과다. 두 요인을 분리하려면 decay를 합=1로 정규화한 버전과도 비교해야 하지만, 이번 5-1의 목적(예전 버그 동작 자체를 재현해 uniform과 비교)에는 부합하므로 추가 실험은 하지 않았다 — 다만 해석 시 이 confound를 감안해야 한다.
...
**1회 관측으로는 가설이 맞는 것처럼 보였다** — lr을 낮춰서 짝지으니 negative sampling이 도움이 되는 것처럼(uniform 기본값 0.0444 대비 HR@20 +14%, 0.0505) 나왔다. **다만 이 0.0505는 이후 재현성 검증(5-4)에서 이상치로 판명났다** — 3회 반복 평균은 +5%대(0.0453)로, 방향은 맞지만 폭은 훨씬 작다. SimpleX 논문의 주장이 약하게는 재현됐다고 볼 수 있지만, 최초 관측한 +14%는 과장된 수치였다.
...
### 5-4. 재현성 검증 — neg=10/lr=0.002의 0.0505는 이상치였다

임베딩 초기화만 seed(42) 고정했고 negative sampling(학습 중 무작위 음성 샘플링) 자체는 여전히 매 실행마다 무작위다. 5-2에서 "확정"했던 `neg_samples=10, lr=0.002`(HR@20=0.0505)를 같은 설정으로 재실행했더니 0.0444가 나와서 재현이 안 됐다 — 이 문제를 확인하려고 uniform 기본값(neg=1)과 `neg=10,lr=0.002`를 각각 3회씩 반복 실행했다.

| 조합 | run1 | run2 | run3 | 평균 | 표준편차 |
|---|---|---|---|---|---|
| uniform 기본(neg=1, lr=0.005) | 0.0451 | 0.0403 | 0.0437 | **0.0430** | 0.0025 |
| neg=10, lr=0.002 | 0.0478 | 0.0437 | 0.0444 | **0.0453** | 0.0022 |

**0.0505는 이상치였다** — neg=10/lr=0.002 3회 중 최댓값(0.0478)도 0.0505에 못 미친다. 다만 완전히 노이즈만은 아니다 — neg=10/lr=0.002 평균(0.0453)이 uniform 평균(0.0430)보다 3회 다 꾸준히 높다(범위는 일부 겹침: 0.0437~0.0451 구간). 방향성은 일관되게 neg=10/lr=0.002가 낫지만, 개선폭이 원래 주장(+14%)보다 훨씬 작은 +5%대이고 n=3이라 통계적으로 강하게 단정할 수준은 아니다. **"약하게 낫다" 정도가 정직한 결론이다.**

### 5차 최종 결론 (재현성 검증 반영, 정정됨)
...
| 버그 수정 후 uniform 기본(layer=2, neg=1), 3회 평균(5차) | 0.0430 (±0.0025) |
| 버그 수정 후 layer=1(1회 관측, 재현성 미검증) | 0.0437 |
| ~~버그 수정 후 neg=10,lr=0.002 (1회 관측)~~ | ~~0.0505~~ → **재현 실패, 3회 평균 0.0453으로 정정** |
| layer=1 + neg=10,lr=0.002 조합(개선 안 됨, 1회 관측) | 0.0382 |
| ALS (#7) | 0.0608 |

**확정 설정**: 구매만, emb_dim=32, lamda=0.02, layer=2, Adam, batch=10000, uniform, **neg_samples=10, lr=0.002** — HR@20≈0.0453(3회 평균, 이전 발표한 0.0505는 이상치로 정정). ALS와의 격차는 약 **1.34배**다(0.0608/0.0453). 이전에 발표했던 1.10배(3~4차, 버그 있는 코드)와 1.20배(5차 1회 관측)는 각각 코드 버그와 재현되지 않은 이상치 위에서 나온 수치였으므로, **이 1.34배가 지금까지 중 가장 신뢰할 수 있는 값**이다. `layer=1`은 1회 관측으로는 uniform 기본값과 거의 비슷했으나(0.0437) 반복 검증은 안 했다 — 여유가 있으면 이것도 3회 반복해 확인할 가치가 있다.

## 방법론적 제약 및 한계

- **1~4차는 레이어 가중치 버그가 있는 상태에서 측정됨**: 5차에서 표준 방식(uniform)으로 고치고 재검증하니 수치가 달라졌다(0.0553→0.0430 3회 평균). `layer`(4차 결과)도 버그 상태에서 나온 값이라 uniform 기준으로 1회 재검증했으나(5-3), 3회 반복 검증은 아직 안 했다.
- **negative sampling 재짝짓기는 재현성 문제로 정정됨(5-4)**: 처음엔 `neg_samples=10, lr=0.002`가 HR@20=0.0505로 가장 좋다고 봤으나, negative sampling에 seed가 없어 재현이 안 됐다 — 3회 반복 평균은 0.0453(±0.0022)이었다. **모든 5차 결과 중 반복 검증(n≥3)을 거친 건 uniform 기본값과 neg=10/lr=0.002 두 조합뿐이다** — layer 재검증, layer_weight_scheme 비교 등 나머지는 1회 관측값이라 추가 검증 없이는 소수점 둘째 자리 수준의 우열 비교는 신뢰하면 안 된다.
- **임베딩 초기화 seed는 5차부터 고정(42)됨**: 1~4차 실행들은 서로 다른 무작위 초기화에서 시작했다 — 그 시점 결과 간 미세한 차이 일부는 이 확률성 때문일 수 있다. 다만 **negative sampling 자체는 여전히 매 학습마다 무작위라 완전한 결정론은 아니다** — 5-4에서 확인했듯 이게 실제로 유의미한 크기의 run-to-run 분산(HR@20 기준 ±0.002~0.0025)을 만든다. ALS도 유사한 한계(`reports/[ML]_#7_ALS_...md`의 "모든 실험이 동일한 test set을 반복 조회")를 가지고 있다.
...
- `configs/LightGCN/params.yaml` (`eval_k_list` 추가, 5차에서 최종 확정값(lr=0.002, neg_samples=10, opt=3, batch=10000, layer_weight_scheme=uniform 등)으로 전체 갱신 — CLI 인자 생략 시 이 기본값으로 실행됨)
...
- `data/outputs/LightGCN/hp_tuning_experiment/variance_check/` (5-4 재현성 검증용 반복 실행 결과, gitignore 대상)
- `data/outputs/LightGCN/PRED_MAIN_RECOMMEND.csv`, `eval_results.csv` (실행할 때마다 덮어써지는 로컬 산출물, gitignore 대상 — **다운스트림에서 쓰기 전엔 최종 확정 설정으로 재실행해서 최신 상태인지 반드시 확인할 것**. 실험 중간에는 마지막으로 돌린 조합 결과가 들어있을 수 있음)
...
- [x] ~~negative sampling × lr 재짝짓기 재실험~~ — 5-2에서 완료, `neg_samples=10, lr=0.002`가 최선 (초기 관측 HR@20=0.0505는 이상치, 5-4 재현성 검증으로 0.0453 정정)
- [x] ~~`layer` 재검증(uniform 기준)~~ — 5-3에서 1회 완료. `layer=1`이 단독으로는 가장 좋지만(0.0437) `neg=10/lr=0.002`와 조합 시 오히려 나빠져(0.0382) `layer=2` 유지로 확정. 단 5-4에서 확인한 run-to-run 분산을 감안하면 이것도 반복 검증(n≥3)이 필요함 — 미완
- [x] ~~negative sampling 재현성 검증~~ — 5-4에서 완료, 3회 반복으로 0.0505가 이상치임을 확인하고 0.0453으로 정정
- [x] ~~`#34`: LightGCN bipartite(#35) vs tri 비교로 페르소나 순수 효과 분리 측정~~ — PR #40에서 완료·병합됨. 결과: emb_dim=32(tri 튜닝 기준)에서는 tri가 우세(HR@20 0.0453 vs bipartite 0.0398, +14%)하지만, emb_dim=64로 재검증하면 차이가 표준편차 내로 사라짐(tri 0.0505 vs bipartite 0.0496) — 페르소나 효과는 하이퍼파라미터에 좌우되는 약하고 불안정한 효과로 결론남. 상세: `reports/[ML]_LightGCN_bipartite_vs_tri_페르소나_효과_검증_2026-07-08.md`
- [ ] (선택) negative sampling에도 seed 고정 — 지금은 embedding 초기화만 재현 가능하고 negative sampling은 매 실행 무작위라 완전한 결정론이 아님. 고치면 5-3(layer)·5-4 같은 반복 검증 비용이 줄어듦
- [x] ~~rec-system 레포 연동 시 `PRED_MAIN_RECOMMEND.csv`에 `twiddler`/`user_type` 컬럼 추가 필요~~ — **정정**: rec-system(`backend/api/schemas.py`, `lightgcn_service.py`, `recommend_main.py`) 확인 결과 `twiddler`/`user_type`/`model_type`은 CSV 컬럼이 아니라 rec-system 서빙 계층이 요청 파라미터·graph_type 기준으로 그때그때 계산해 응답에 붙이는 값이었다. CSV는 `user_id`/`item_id`/`score`/`rank`만 있으면 됨. 실제 남은 문제는 아래 항목.
- [ ] rec-system 연동 — retail 쪽은 `#34` 후속 작업으로 `PRED_MAIN_RECOMMEND_tripartite.csv`/`_bipartite.csv`로 파일명 분리 완료(`save_recommendations.py::resolve_rec_filename`). 단 `rec-system/backend/api/services/lightgcn_service.py`는 아직 고정 경로 `PRED_MAIN_RECOMMEND.csv` 하나만 읽고 graph_type="tripartite"는 not_implemented로 하드코딩되어 있음 — rec-system 쪽 서빙 로직을 두 파일 경로로 나눠 읽도록 고쳐야 실제 연동이 완료됨 (별도 세션/이슈로 진행 권장)
