...
| **버그 수정 후 uniform 기본**(5차) | 0.0444 |
| **버그 수정 후 neg=10,lr=0.002**(5차 최고) | **0.0505** |
| ALS (#7) | 0.0608 |

버그 수정 후 최고 기록(0.0505) 기준 ALS와의 격차는 약 **1.20배**다(버그 있던 3~4차의 1.10배보다는 살짝 벌어졌지만, 애초에 그 1.10배가 잘못된 코드 위에서 나온 숫자였다). `layer` 재검증(uniform 기준, layer∈{1,2,3,4})은 진행 중 — 완료되면 이 표에 추가한다.
...
- [ ] `layer` 재검증 (uniform 기준, layer∈{1,2,3,4}) — #37 진행 중, 4차는 버그 상태에서 측정된 값이라 재확인 필요
- [ ] #34: LightGCN bipartite(#35) vs tri 비교로 페르소나 순수 효과 분리 측정 — 버그 수정 후 최종 확정 조합(구매만, emb_dim=32, neg=10, lr=0.002, lamda=0.02, layer=2, Adam, batch=10000, uniform)을 bipartite에도 동일 적용해야 공정한 비교, #34 코멘트 갱신 예정
