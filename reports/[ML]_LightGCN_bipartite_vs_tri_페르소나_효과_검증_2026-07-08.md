...
## 6. 탐색적 분석 — 저활동 유저에게 더 도움되는가 (1회 실행, 확정 아님)

전체 평균 HR@20이 페르소나 효과를 보기에 안 좋은 지표일 수 있다는 가설을 검증했다 — 페르소나(행동 기반 라벨)는 이론적으로 협업 필터링 신호가 부족한 저활동 유저(train 구매 이력이 적은 유저)에게 더 유용해야 하고, 헤비 유저는 이미 구매 이력만으로 충분해서 차이가 안 날 수 있다.

emb_dim=64 예측 결과(tri/bipartite 각 1회, 재학습 없이 기존 `evaluate_lightgcn.py` 순수 함수 재사용)를 유저별 train 구매 건수 사분위수로 4구간 나눠 재평가했다.

| 구간 (train 구매 건수) | 유저 수 | tri HR@20 | bipartite HR@20 | 차이 (tri−bi) |
|---|---|---|---|---|
| Q1 (0~1건, 최저활동) | 544 | 0.0643 | 0.0588 | **+0.0055** |
| Q2 (2건) | 225 | 0.0622 | 0.0578 | +0.0044 |
| Q3 (3~4건) | 381 | 0.0341 | 0.0367 | −0.0026 |
| Q4 (5건 이상, 최고활동) | 315 | 0.0413 | 0.0381 | +0.0032 |

**약한 신호는 있다** — 4구간 중 tri가 가장 크게 앞서는 곳이 최저활동 구간(Q1, +0.0055)이라 가설과 방향이 맞는다. 다만 Q3에서는 오히려 bipartite가 이겨서 단조적인 패턴은 아니다. 구간당 유저 수가 225~544명으로 전체(1,465명)보다 훨씬 적고, 이 정도 표본에서는 이미 확인된 run-to-run 노이즈 수준(±0.002~0.005)과 구분하기 어렵다 — **1회 실행 기반 탐색적 신호일 뿐, 반복 검증 전까지는 결론으로 쓰지 않는다.**
