...
> ⚠️ **재검토 중**: tri-tuned 하이퍼파라미터(emb_dim=32) 기준으로는 tri가 bipartite보다 일관되게 좋았다(HR@20 0.0453 vs 0.0398). 그런데 bipartite에 `emb_dim=64`를 써봤더니 1회 실행에서 0.0485로 tri 평균을 넘어섰다 — bipartite가 tri에 안 맞는 설정 때문에 저평가됐을 가능성이 있어, 반복 실행으로 확인 중이다. **아래 결론은 확정 전이다.**
...
- **bipartite는 tri-tuned 하이퍼파라미터로만 평가함**: `#37`에서 tri에 맞춰 튜닝한 설정(emb_dim=32, layer=2 등)을 bipartite에도 그대로 적용했다. 의도적인 선택이다 — 페르소나 유무 하나만 바꾸고 나머지를 전부 고정해야 결과 차이를 페르소나 탓으로 돌릴 수 있는 ablation 설계이기 때문이다. 다만 이건 "bipartite가 낼 수 있는 최선의 성능"이 아니라 "tri에 최적화된 조건 아래서의 bipartite 성능"이라는 뜻이다 — bipartite는 페르소나 노드가 없어 그래프가 더 단순해서, 더 작은 emb_dim이나 다른 layer 수가 자체적으로는 더 잘 맞을 가능성이 있다. 아래 5-4에서 이 우려에 대한 저비용 로버스트니스 체크를 진행했다.
