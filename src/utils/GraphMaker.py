...
def make_app(
    ...
    for uidx in app_scope_idxs:
        ...
        ps_approach = [
            whole_personas_2[pidx] for pidx in ps_approach_pidxs
    ...
    for uidx, ps in multi_labels_approach.items():
        ...
        ps_idx = [
            persona2idx_whole[p] for p in ps
        ]  # use the original persona2idx_whole to make the transformation
        tri_graph_uidx2pidx_approach[uidx] = ps_idx

    tri_graph_uidx2pidx_approach = {
        int(k): [int(e) for e in v] for k, v in tri_graph_uidx2pidx_approach.items()
    }  # reform the datatype
