...
class model_LGCN_afd_tri(object):
    def __init__(self, n_users, n_items, n_personas, lr, lamda, emb_dim, layer, pre_train_latent_factor, graph_embeddings, graph_conv,
        ...
        if graph_conv == '1D': self.graph_emb = graph_embeddings # bases, M+N+P * F
        ...
        if self.if_pretrain:
            ...
            self.persona_embeddings = tf.Variable(tf.random.normal([self.n_personas, self.emb_dim], mean=0.01, stddev=0.02, dtype=tf.float32), name='persona_embeddings') # initialize samely
        ...
        # defaultly sum
        if self.pooling == 'Sum': self.layer_weight = [(1 / (l + 1)) ** 1 for l in range(self.layer + 1)] # weights of different layer embeddings
        if self.pooling[0: 3] == 'MLP':
            ...
            self.pooling_W = [tf.Variable(tf.random.normal([self.pooling_layer_size[l], self.pooling_layer_size[l + 1]], mean=0, stddev=0.01, dtype=tf.float32)) for l in range(self.pooling_mlp_layer)]
            self.pooling_b = [tf.Variable(tf.random.normal([self.pooling_layer_size[l + 1]], mean=0, stddev=0.01, dtype=tf.float32)) for l in range(self.pooling_mlp_layer)]
        if self.prediction[0: 3] == 'MLP':
            ...
            self.prediction_W = [tf.Variable(tf.random.normal([self.prediction_layer_size[l], self.prediction_layer_size[l + 1]], mean=0, stddev=0.01, dtype=tf.float32)) for l in range(self.prediction_mlp_layer)]
            self.prediction_b = [tf.Variable(tf.random.normal([self.prediction_layer_size[l + 1]], mean=0, stddev=0.01, dtype=tf.float32)) for l in range(self.prediction_mlp_layer)]

        ## convolutional layers definition
        self.embeddings = tf.concat([self.user_embeddings, self.item_embeddings, self.persona_embeddings], axis=0) # newly added       # todo: add persona item embeddings
        ...
        for l in range(self.layer):
            ...
            if self.graph_conv == '1D': self.embeddings = tf.matmul(tf.matmul(self.graph_emb, tf.linalg.tensor_diag(self.kernel[l])), tf.matmul(self.graph_emb, self.embeddings, transpose_a=True, transpose_b=False))
            else:
                self.embeddings_U, self.embeddings_V = tf.split(self.embeddings, [self.n_users, self.n_items], axis=0)
                self.embeddings_U = tf.matmul(tf.matmul(self.graph_emb_U, tf.linalg.tensor_diag(self.kernel_U[l])), tf.matmul(self.graph_emb_U, self.embeddings_U, transpose_a=True, transpose_b=False))
                self.embeddings_V = tf.matmul(tf.matmul(self.graph_emb_V, tf.linalg.tensor_diag(self.kernel_V[l])), tf.matmul(self.graph_emb_V, self.embeddings_V, transpose_a=True, transpose_b=False))
        ...
        self.user_all_embeddings, self.item_all_embeddings, self.persona_all_embeddings = tf.split(self.all_embeddings, [self.n_users, self.n_items, self.n_personas], 0)
        ...
        if self.prediction == 'InnerProduct': # our focus
            ...
            # the all_ratings is for inference
        ...
        self.all_ratings += self.items_in_train_data   ## set a very large negative value for the items appearing in the training set to make sure they are at the end of the sorted list
        ...
        if 'Regularization' in self.generalization:
            ...
            self.reg_list = [self.u_embeddings_reg, self.pos_i_embeddings_reg, self.neg_i_embeddings_reg, self.persona_embeddings] # we directly add the persona_embeddings to here
        ...
        ## update parameters
        self.var_list = [self.user_embeddings, self.item_embeddings, self.persona_embeddings]  ## learnable parameter list
    ...
    def cross_entropy_loss(self, pos_scores, neg_scores):
        maxi = tf.math.log(tf.nn.sigmoid(pos_scores)) + tf.math.log(1 - tf.nn.sigmoid(neg_scores))
        loss = tf.negative(tf.reduce_sum(maxi))
        return loss
    ...
    # no need for the inner production-based prediction
