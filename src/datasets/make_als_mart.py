...
def generate_user_item_datamart():
    ...
    cart_items = events[events["event_type"] == "add_to_cart"][
    ...
    datamart_all = (
        df_mart.groupby(["user_id", "item_id"])
        .agg(timestamp=("timestamp", "max"), total_score=("score", "sum"))
        .reset_index()
    )
    ...
    datamart_usa = (
        df_mart_usa.groupby(["user_id", "item_id"])
        .agg(timestamp=("timestamp", "max"), total_score=("score", "sum"))
        .reset_index()
    )
