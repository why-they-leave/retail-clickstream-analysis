...
def _step1_single(client, grouped_df, user_ids, i, users_per_iteration, model):
    """Step 1 단일 반복: 100명 샘플 → 20개 후보."""
    user_data = sample_user_descriptions(grouped_df, user_ids, n=users_per_iteration, seed=i * 7)
    response = call_llm(client, STEP1_SYS, STEP1_USER + user_data, model=model)
    return {"iter": i, "personas": parse_persona_list(response), "raw": response}


def step1(
    client, grouped_df, user_ids, n_iter=40, users_per_iteration=100, workers=8, model="solar-pro"
):
    """Step 1: 40회 반복, 각 100명 샘플로 20개 후보 생성."""
    logger.info("[Step 1] %d회 반복 실행 중 (%d명 샘플)...", n_iter, users_per_iteration)
    tasks_args = [
        (client, grouped_df, user_ids, i, users_per_iteration, model) for i in range(n_iter)
    ]
    results = run_parallel(_step1_single, tasks_args, max_workers=workers, desc="Step 1")
    return sorted(results, key=lambda x: x["iter"])


def _step2_single(client, all_sets, i, sets_per_sample, model):
    """Step 2 단일 반복: 5세트 샘플링 → 정제된 20개."""
    random.seed(i * 13)
    sampled = random.sample(all_sets, sets_per_sample)
    context = format_persona_sets(sampled)
    response = call_llm(client, STEP2_SYS, STEP2_USER + context, model=model)
    return {"iter": i, "personas": parse_persona_list(response), "raw": response}


def step2(client, step1_results, n_iter=8, sets_per_sample=5, workers=4, model="solar-pro"):
    """Step 2: 40세트 중 5세트씩 샘플링, 8회 반복으로 정제."""
    logger.info("[Step 2] %d회 반복 실행 중 (5세트씩 샘플링)...", n_iter)
    all_sets = [r["personas"] for r in step1_results]
    tasks_args = [(client, all_sets, i, sets_per_sample, model) for i in range(n_iter)]
    results = run_parallel(_step2_single, tasks_args, max_workers=workers, desc="Step 2")
    return sorted(results, key=lambda x: x["iter"])


def step3(client, step2_results, model="solar-pro"):
    """Step 3: 8세트(160개) → 최종 20개 페르소나."""
    logger.info("[Step 3] 최종 20개 페르소나 확정 중...")
    all_sets = [r["personas"] for r in step2_results]
    context = format_persona_sets(all_sets)
    response = call_llm(client, STEP3_SYS, STEP3_USER + context, model=model)
    return parse_persona_list(response), response
...
def parse_args():
    parser = ArgumentParser(description="Generate persona taxonomy from purchase histories.")
...
def main():
    ...
    s1 = step1(
        ...
        grouped_df,
        user_ids,
        n_iter=int(persona["n_iterations"]),
        users_per_iteration=int(persona["users_per_iteration"]),
        workers=workers,
        model=model,
    )
    with open(output_dir / "step1_persona_sets.json", "w", encoding="utf-8") as f:
        json.dump(s1, f, ensure_ascii=False, indent=2)
    logger.info("→ Step 1 저장: %s", output_dir / "step1_persona_sets.json")

    s2 = step2(
        client,
        s1,
        n_iter=int(persona["step2_iterations"]),
        sets_per_sample=int(persona["sets_per_step2_sample"]),
        workers=workers,
        model=model,
    )
    with open(output_dir / "step2_persona_sets.json", "w", encoding="utf-8") as f:
        json.dump(s2, f, ensure_ascii=False, indent=2)
    logger.info("→ Step 2 저장: %s", output_dir / "step2_persona_sets.json")

    final_personas, raw = step3(client, s2, model=model)
    ...
    with open(output_dir / "final_personas_raw.txt", "w", encoding="utf-8") as f:
        f.write(raw)
    logger.info("→ Step 3 저장: %s", output_dir / "final_personas.json")

    logger.info("=== 최종 페르소나 목록 (%d개) ===", len(final_personas))
    for i, p in enumerate(final_personas, 1):
        logger.info("%2d. %s — %s", i, p["name"], p["definition"])
