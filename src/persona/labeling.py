...
def main():
    ...
    [mba_df, user_ids, user_num, user_ids_kv,
     item_names, item_num, items_kv, G_user, G_item] = mba_ds.MBA_load_data(DS_PATH)
    ...
    with ThreadPoolExecutor(max_workers=12) as ex:
        futures = [
            ex.submit(
                assign_user_label, client, sys_u, usr_u,
                user_ids[uidx], grouped_df, tri_graph_uidx2tidx_train[uidx]
            )
    ...
    with ThreadPoolExecutor(max_workers=12) as ex:
        futures = [
            ex.submit(assign_item_label, client, sys_i, usr_i, name)
